...
- ``encode_array(items, fp, format="json"|"xml")``   stream a large collection element-by-element
- ``decode_array(fp, format="json"|"xml") -> iterator`` parse a streamed collection lazily, one element at a time
...
library. ``format="xml"`` streams a typed, round-trippable XML representation for
which the standard DOM parsers would otherwise materialize the whole tree.
"""
